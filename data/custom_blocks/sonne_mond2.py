...
try:
    ...
    try:
        import subprocess
        subprocess.check_call(['pip', 'install', 'ephem'])
        import ephem
        HAS_EPHEM = True
        logger.info("ephem wurde automatisch installiert")
    except Exception:
        HAS_EPHEM = False
        logger.warning("ephem konnte nicht installiert werden, verwende Fallback-Berechnung")
...
def _fallback_sun_times(lat, lon, date=None):
    ...
    # MEZ/MESZ korrekt bestimmen
    # MESZ: letzter Sonntag im März 02:00 UTC bis letzter Sonntag im Oktober 03:00 UTC
    def _is_mesz(dt):
        year = dt.year
        ...
        return mesz_start <= dt < mesz_end
    
    utc_offset = 2 if _is_mesz(date) else 1  # MESZ = UTC+2, MEZ = UTC+1
