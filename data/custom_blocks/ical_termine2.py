...
class ICalTermine(LogicBlock):
    ...
    def on_start(self):
        ...
        self.set_debug('Version', self.VERSION)
        self.set_debug('Status', 'Initialisiert')
        self.set_debug('Auto-Trigger', '(nicht konfiguriert)')
            ...
            while self._running_flag:
                ...
                if now.hour == target[0] and now.minute == target[1]:
                    ...
                    if key != self._last_auto_key:
                        ...
                        self.set_debug('Last Auto-Trigger', now.strftime('%d.%m.%Y %H:%M:%S'))
    ...
    async def _fetch_and_process(self):
        ...
        if not url:
            ...
            self.set_debug('Status', 'Fehler – keine URL')
            return

        self.set_debug('Status', 'Daten abrufen…')
        self.set_debug('URL', url[:80])
        ...
        try:
            ...
            self.set_debug('Status', f'Fehler: {e}')
            return
        ...
        try:
            ...
            self.set_debug('Status', f'Parse-Fehler: {e}')
            return

        # 3) Filter future events, sort by date
        now_dt = datetime.now()
        ...
        self.set_debug('Events gesamt', str(len(events)))
        self.set_debug('Zukünftige Events', str(len(future)))
        ...
        self.set_debug('Status', f'OK – {len(future)} Termine')
        self.set_debug('Last Update', datetime.now().strftime('%d.%m.%Y %H:%M:%S'))

    # ------------------------------------------------------------------ helpers
    async def _download(self, url: str, timeout: int = 15) -> str:
        """Download iCal data via HTTP(S)."""
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=timeout),
                                   ssl=False) as resp:
                resp.raise_for_status()
                raw = await resp.read()
                # Try UTF-8 first, then ISO-8859-15
